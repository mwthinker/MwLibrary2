...
class BatchTriangles : public mw::Batch<TestShader2> {
public:
	...
	void draw() const override {
		if (mw::Window::getOpenGlMajorVersion() >= 2) {
			...
			for (const TestShader2::Vertex& v : *this) {
				...
				if (texture) {
					glTexCoord2f(v.tex_.x_, v.tex_.y_);
				}
				glVertex2f(v.pos_.x_, v.pos_.y_);
				glColor4f(v.color_.red_, v.color_.green_, v.color_.blue_, v.color_.alpha_);
